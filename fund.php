...
<form method="post" action="" enctype="multipart/form-data" onsubmit="return validateform()">
	<input type="hidden" name="fund_raiser_id" id="fund_raiser_id" value="<?php echo isset($_GET['editid']) ? $rsedit['fund_raiser_id'] : 0; ?>">
	<div class="row">
...
<div class="row">
	<div class="col-md-2" style="padding-top: 5px;">Fund Raiser Contact</div>
	<div class="col-md-10">
		<input type="text" name="fund_raiser_contact"  id="fund_raiser_contact" class="form-control" value="<?php echo $commonclass->formatNumber($rsedit['fund_raiser_contact'],0); ?>">
...
<div class="row">
	...
	<div class="col-md-10">
		<input type="text" name="fund_amount"  id="fund_amount" class="form-control" value="<?php echo $commonclass->formatNumber($rsedit['fund_amount'],0); ?>">
		<span id="errfund_amount" class="errorclass"></span>
	</div>
</div>
...
<div class="row">
		<div class="col-md-2" style="padding-top: 5px;">Hospital/Orphanage Name </div>
		<div class="col-md-10">
			<input type="text" name="hospital_name"  id="hospital_name" class="form-control" value="<?php echo $rsedit['hospital_name']; ?>">
			<span id="errtitle" class="errorclass"></span>
		</div>
	</div>
	...
	<br>
<div class="row">
	<div class="col-md-2" style="padding-top: 5px;">Hospital/Orphanage Contact</div>
	<div class="col-md-10">
		<input type="text" name="hospital_contact"  id="hospital_contact" class="form-control" value="<?php echo $commonclass->formatNumber($rsedit['hospital_contact'],0); ?>">
		<span id="errfund_amount" class="errorclass"></span>
	</div>
</div>
		...
		<select class="form-control" name="status" id="status">
			...
			foreach($arr as $val ){
				if($val == $rsedit['status']){
					echo "<option value='$val' selected>$val</option>";} else {echo "<option value='$val'>$val</option>";
				}
			}
			?>
...
function validateform(){
	...
	if (!$("#title").val().match(alphaSpaceNumericExp)) {
	  $("#errtitle").html("Entered title not valid...");
	  i = 1;
	}
	if ($("#title").val() == "") {
	  $("#errtitle").html("Kindly enter title...");
	  i = 1;
	}
	if ($("#fund_raiser_id").val() == 0) {
		if ($("#banner_img").val() == "") {
		  $("#errbanner_img").html("Kindly enter banner image...");
		  i = 1;
		}
	}
	if ($("#fund_raiser_description").val() == "") {
	  $("#errfund_raiser_description").html("Kindly enter fund raiser description...");
	  i = 1;
	}
	if ($("#start_date").val() == "") {
	  $("#errstart_date").html("Kindly enter the Start date...");
	  i = 1;
	}
	if ($("#end_date").val() == "") {
	  $("#errend_date").html("Kindly enter End date...");
	  i = 1;
	}
	if (!$("#fund_amount").val().match(numericExp)) {
	  $("#errfund_amount").html("Entered Fund Amount is not valid...");
	  i = 1;
	}
	if ($("#fund_amount").val() == "") {
	  $("#errfund_amount").html("Kindly enter the Fund amount...");
	  i = 1;
	}
	if ($("#status").val() == "") {
